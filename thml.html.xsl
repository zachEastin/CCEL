...
<xsl:variable name="bookInfo" select="bookInfo.xml"/>
...
<table id="bcbHide" align="center" style="clear:right; margin-top:0in">
  ...
  <tr>
	...
	# Must Comment Out: running on CCEL desktop
	<xsl:when test="$desktop=1">
	 ...
	 # Must Comment Out: <xsl:if test="$cd!=1">
	    ...
	    # Must Comment Out: </xsl:if>
        </xsl:when>

	# Must Comment Out: create HTML files on server
        <xsl:otherwise>
...
<!-- include included stuff -->
  ...
  <xsl:template match="scripCom" mode="includeParsing">
    <a name="{@id}">[[@Bible:<xsl:value-of select="@passage"/>]]</a>
  </xsl:template>
...
<xsl:template match="span" mode="includeParsing">
  <span>
    ...
    <xsl:choose>
      <xsl:when test="@lang">\
        <xsl:value-of select="concat('[[@Headword+', @lang, ':', ., ']]')"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:apply-templates mode="includeParsing"/>
      </xsl:otherwise>
